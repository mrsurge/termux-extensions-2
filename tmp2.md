# Framework Shells Fork — Implementation Plan v1
...
def require_auth(
    ...
    x_framework_key: str = Header(None),  # TE2 compat
) -> None:
    secret = get_secret()
    if not secret:
    ...
    elif x_framework_key:
        token = x_framework_key  # TE2 compatibility
...
├── pty.py            # PTY helpers (optional dtach later)
├── cli/
│   ├── __init__.py
│   └── main.py       # Click CLI
└── api/
...
def get_te2_router():
    """Create FastAPI router with TE2 compat headers."""
    # X-Framework-Key alias handled in auth.py
