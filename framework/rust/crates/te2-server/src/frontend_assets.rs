...
async fn app_shell(
    ...
    if app_id != app.app_id {
        let mut canonical_url = format!("/app/{}", app.app_id);
    ...
    let content = template
        .replace(
            "{{ app_id|tojson }}",
            &serde_json::to_string(&app.app_id).unwrap_or_else(|_| "\"\"".to_owned()),
        )
        .replace(
...
async fn serve_extension_file(
    ...
    // directories are deliberately outside this server's compatibility target.
