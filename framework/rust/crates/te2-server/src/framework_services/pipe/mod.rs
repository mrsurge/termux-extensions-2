...
mod tests {
    ...
    fn test_root(name: &str) -> PathBuf {
        ...
        root.push(format!("te2-server-pipe-{name}-{}", std::process::id()));
    ...
    fn request(method: &str, params: serde_json::Value, root: &std::path::Path) -> PipeEnvelope {
        PipeEnvelope {
            ...
            origin_nid: 1100,
            origin_name: "code_te2.explorer".to_owned(),
    ...
    async fn dispatches_git_clone_start_and_routes_progress_notifications() {
        ...
        assert_eq!(terminal.target_name.as_deref(), Some("code_te2.explorer"));
    ...
    async fn dispatches_run_target_registration_and_release() {
        ...
        let registered = dispatch_request(
            targeted_request(
                ...
                json!({
                    "ownerId": "runner-profile:code_te2:project:web",
                    "shellId": "shell-4173",
                    "port": 4173,
        ...
        assert_eq!(
            ...
            Some("runner-profile:code_te2:project:web")
        );
        assert_eq!(
