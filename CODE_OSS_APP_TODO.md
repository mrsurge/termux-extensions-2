1. [x] **File/Project History Tabs (d)**  
   ...
   - **Implementation**: History still lives in `code_oss_history.json`, but editor/UI state (including last-open file) now persists via `preferences_store.py` and `/api/app/code_oss/preferences`, so the CM6 tabs and reload behaviour are stable across browsers.
...
2. [ ] **Restore Menu Actions & Auto-Save Toggle (e)**  
   ...
   - *How*:  
     ...
     - Add an auto-save toggle in the disk-backed preferences; when off, surface a dirty indicator and enable Save.  
   - *Tweaks*: throttle menu buttons while a command is in flight to avoid duplicate posts.

3. [x] **Richer Explorer Drawer (b)**  
   - ~~*Why mid-tier*: Needs extra data (git status, separators) but can piggyback on existing event flow with some backend help.~~  
   - ~~*How*:~~  
     - ~~Extend backend to run lightweight `git status --porcelain`; cache results and attach them to bridge `explorerTree` payloads.~~  
   ...
   - **Implementation**: `_gather_git_status()` now annotates explorer events with status + executable flags; the drawer renders separators, badges, and green executables with a persisted “Git indicators” toggle.
