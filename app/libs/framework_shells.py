...
class FrameworkShellManager:
    ...
    async def _launch_pty(self, record: ShellRecord) -> ShellRecord:
        ...
        # Configure PTY to raw mode to avoid canonical line buffering limits (ICANON).
        # This helps prevent input/output surprises around ~4096 byte boundaries.
        try:
            attrs = termios.tcgetattr(slave_fd)
            # iflag: Clear ICRNL (translate CR->NL), IXON (flow control)
    ...
    async def write_to_pty(self, shell_id: str, data: bytes | str) -> None:
        async with self._get_lock():
            ...
            try:
                # Chunk writes to handle PTY buffer limits (often ~4096 bytes) and partial writes.
                # os.write returns the number of bytes written.
                offset = 0
                total = len(payload)
                while offset < total:
                ...
                _shell_debug("PTY write", f"complete shell={shell_id}")
