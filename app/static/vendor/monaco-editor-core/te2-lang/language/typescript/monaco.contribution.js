...
// src/languages/features/typescript/lib/typescriptServicesMetadata.ts
...
import { editor as editor2 } from "monaco-editor-core";
var WorkerManager;
          ...
          this._client = (async () => {
            ...
            if (this._defaults.getEagerModelSync()) {
              return await this._worker.withSyncedResources(
                editor2.getModels().filter((model) => model.getLanguageId() === this._modeId).map((model) => model.uri)
              );
            }
...
// src/languages/features/typescript/lib/lib.index.ts
var libFileSet;
...
var Adapter, LibFiles, DiagnosticsAdapter, SuggestAdapter, SignatureHelpAdapter, QuickInfoAdapter, DocumentHighlightAdapter, DefinitionAdapter, ReferenceAdapter, OutlineAdapter, Kind, outlineTypeTable, FormatHelper, FormatAdapter, FormatOnTypeAdapter, CodeActionAdaptor, RenameAdapter, InlayHintsAdapter;
  ...
  "src/languages/features/typescript/languageFeatures.ts"() {
    ...
    LibFiles = class {
      ...
      getOrCreateModel(fileName) {
        const uri = Uri2.parse(fileName);
        const model = editor3.getModel(uri);
        if (model) {
          return model;
        }
        if (this.isLibFile(uri) && this._hasFetchedLibFiles) {
          return editor3.createModel(this._libFiles[uri.path.slice(1)], "typescript", uri);
        }
        const matchedLibFile = typescriptDefaults.getExtraLibs()[fileName];
        if (matchedLibFile) {
          return editor3.createModel(matchedLibFile.content, "typescript", uri);
        }
        return null;
      }
    ...
    DiagnosticsAdapter = class extends Adapter {
      constructor(_libFiles, _defaults, _selector, worker) {
          ...
          const visibleSubscription = model.onDidChangeAttached(() => {
            ...
            if (onlyVisible) {
              if (model.isAttachedToEditor()) {
                ...
                editor3.setModelMarkers(model, this._selector, []);
              }
            }
          });
        ...
        this._disposables.push(
          editor3.onDidCreateModel((model) => onModelAdd(model))
        );
        this._disposables.push(editor3.onWillDisposeModel(onModelRemoved));
        this._disposables.push(
          editor3.onDidChangeModelLanguage((event) => {
        ...
        this._disposables.push({
          dispose() {
            for (const model of editor3.getModels()) {
        ...
        editor3.getModels().forEach((model) => onModelAdd(model));
      }
      dispose() {
      ...
      async _doValidate(model) {
        ...
        const relatedUris = diagnostics.map((d) => d.relatedInformation || []).reduce((p, c) => c.concat(p), []).map(
          (relatedInformation) => relatedInformation.file ? Uri2.parse(relatedInformation.file.fileName) : null
        );
        await this._libFiles.fetchLibFilesIfNecessary(relatedUris);
        if (model.isDisposed()) {
          return;
        }
        editor3.setModelMarkers(
          model,
          this._selector,
    ...
    SuggestAdapter = class _SuggestAdapter extends Adapter {
...
${tagToString(tag)}`;
    ...
    SignatureHelpAdapter = class _SignatureHelpAdapter extends Adapter {
      constructor() {
        super(...arguments);
    ...
    DefinitionAdapter = class extends Adapter {
      ...
      async provideDefinition(model, position, token) {
        ...
        await this._libFiles.fetchLibFilesIfNecessary(
          entries.map((entry) => Uri2.parse(entry.fileName))
        );
        if (model.isDisposed()) {
          return;
        }
    ...
    FormatHelper = class extends Adapter {
...
// src/languages/features/typescript/tsMode.ts
var tsMode_exports = {};
...
import { languages as languages2 } from "monaco-editor-core";
...
function setupMode(defaults, modeId) {
  ...
  function registerProviders() {
    ...
    if (modeConfiguration.completionItems) {
      providers.push(
        languages2.registerCompletionItemProvider(
          modeId,
    ...
    if (modeConfiguration.signatureHelp) {
      providers.push(
        languages2.registerSignatureHelpProvider(
          modeId,
    ...
    if (modeConfiguration.hovers) {
      providers.push(
        languages2.registerHoverProvider(modeId, new QuickInfoAdapter(worker))
      );
    }
    if (modeConfiguration.documentHighlights) {
      providers.push(
        languages2.registerDocumentHighlightProvider(
          modeId,
    ...
    if (modeConfiguration.definitions) {
      providers.push(
        languages2.registerDefinitionProvider(
          modeId,
    ...
    if (modeConfiguration.references) {
      providers.push(
        languages2.registerReferenceProvider(
          modeId,
    ...
    if (modeConfiguration.documentSymbols) {
      providers.push(
        languages2.registerDocumentSymbolProvider(
          modeId,
    ...
    if (modeConfiguration.rename) {
      providers.push(
        languages2.registerRenameProvider(
          modeId,
    ...
    if (modeConfiguration.documentRangeFormattingEdits) {
      providers.push(
        languages2.registerDocumentRangeFormattingEditProvider(
          modeId,
    ...
    if (modeConfiguration.onTypeFormattingEdits) {
      providers.push(
        languages2.registerOnTypeFormattingEditProvider(
          modeId,
    ...
    if (modeConfiguration.codeActions) {
      providers.push(
        languages2.registerCodeActionProvider(modeId, new CodeActionAdaptor(worker))
      );
    }
    if (modeConfiguration.inlayHints) {
      providers.push(
        languages2.registerInlayHintsProvider(modeId, new InlayHintsAdapter(worker))
      );
    }
...
import { languages as languages3, Emitter } from "monaco-editor-core";
function getMode() {
  return Promise.resolve().then(() => (init_tsMode(), tsMode_exports));
}
var ModuleKind, JsxEmit, NewLineKind, ScriptTarget, ModuleResolutionKind, LanguageServiceDefaultsImpl, typescriptVersion2, modeConfigurationDefault, typescriptDefaults, javascriptDefaults, getTypeScriptWorker2, getJavaScriptWorker2;
var init_register = __esm({
  "src/languages/features/typescript/register.ts"() {
    ...
    getTypeScriptWorker2 = () => {
      return getMode().then((mode) => mode.getTypeScriptWorker());
    };
    getJavaScriptWorker2 = () => {
      return getMode().then((mode) => mode.getJavaScriptWorker());
    };
    languages3.onLanguage("typescript", () => {
      return getMode().then((mode) => mode.setupTypeScript(typescriptDefaults));
    });
    languages3.onLanguage("javascript", () => {
