...
.te-fp-menu-group[data-open="true"] .te-fp-menu-dropdown {
  display: flex;
}
.te-fp-menu-dropdown button {
  background: transparent;
  border: none;
  color: var(--foreground, #f5f5f5);
  padding: 8px 14px;
...
.te-fp-menu-dropdown button:hover {
  background: rgba(80, 120, 255, 0.18);
}
.te-fp-menu-dropdown .te-fp-menu-empty {
  padding: 8px 14px;
  font-size: 0.9rem;
  color: var(--muted-foreground, #aaa);
}
    ...
    <div class="te-fp-dialog">
      ...
      <div class="te-fp-body">
        ...
        <div class="te-fp-options">
          ...
          <div class="te-fp-menu-group">
            <button type="button" class="te-fp-menu-button" aria-label="Bookmarks" title="Bookmarks">🔖</button>
            <div class="te-fp-menu-dropdown">
...
async function getBookmarks() {
  try {
    ...
    if (bookmarks && bookmarks.ok && Array.isArray(bookmarks.data)) {
      state.bookmarks = bookmarks.data;
      renderBookmarkMenu();
    }
  } catch (err) {
    console.error('[FilePicker] Failed to load bookmarks', err);
  }
}
...
function renderBookmarkMenu() {
  ...
  state.bookmarks.forEach(bookmark => {
    ...
    btn.addEventListener('click', () => {
      ...
      if (elements.bookmarkMenu) {
        elements.bookmarkMenu.dataset.open = 'false';
      }
    });
    elements.bookmarkDropdown.appendChild(btn);
  });
}
...
function init() {
  ...
  elements = {
    ...
    bookmarkMenu: rootEl.querySelector('.te-fp-menu-group'),
    bookmarkBtn: rootEl.querySelector('.te-fp-menu-button'),
    bookmarkDropdown: rootEl.querySelector('.te-fp-menu-dropdown'),
  };
  ...
  if (elements.bookmarkBtn) {
    elements.bookmarkBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      const isOpen = elements.bookmarkMenu.dataset.open === 'true';
      elements.bookmarkMenu.dataset.open = isOpen ? 'false' : 'true';
    });
  }
