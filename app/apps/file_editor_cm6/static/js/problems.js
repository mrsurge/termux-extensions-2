...
export function createProblemsPanel(options = {}) {
  ...
  // ── State ──
  let currentDetail = {};                // { absPath: markers[] } — authoritative data
  let activeFilePath = '';               // absPath of the currently open file
  let visible = false;
  const collapseState = new Map();       // absPath → boolean (true = collapsed)
  const groupElements = new Map();       // absPath → { group, header, items, badgeErr, badgeWarn, chevron }
  let emptyEl = null;                    // reference to the "No problems" placeholder

  // ── Marker item DOM creation ──
  function _createMarkerItem(absPath, m) {
  ...
  function _createFileGroup(absPath, markers) {
    ...
    const ref = { group, header, items: itemsDiv, badgeErr, badgeWarn, chevron };
    groupElements.set(absPath, ref);

    return group;
  }

  function _updateBadges(badgeErr, badgeWarn, markers) {
  ...
  function _showEmptyPlaceholder() {
    if (emptyEl) return;
  ...
  // ── Diff-based update ──
  function _diffUpdate(nextDetail) {
    ...
    for (const absPath of nextPaths) {
      ...
      const itemNodes = ref.items.querySelectorAll('.problems-item');
  ...
  function _sortGroups() {
    ...
    for (const [, ref] of entries) {
      container.appendChild(ref.group);
    }
  }
  ...
  function _fullRender() {
    ...
    for (const absPath of paths) {
      container.appendChild(_createFileGroup(absPath, currentDetail[absPath]));
    }
  }
  ...
  return {
    ...
    setActiveFile(absPath) {
      ...
      if (prev && groupElements.has(prev)) {
        ...
        if (!collapseState.has(prev) || collapseState.get(prev) === false) {
