/*
 * TE2 completion vendor shim. Full upstream VS Code suggestModel.ts is
 * preserved below as comments; the live exports here own the suggest-session
 * context normalization used by the completion provider bridge.
 * Source: worktrees/vscode-te2-diff/src/vs/editor/contrib/suggest/browser/suggestModel.ts
 */

import { VscodeCompletionTriggerKind } from './languages.js';

export interface VscodeSuggestBridgeContext {
  triggerKind?: number;
  triggerCharacter?: string;
}

export interface VscodeSuggestBridgeTriggerKinds {
  TriggerCharacter?: number;
  TriggerForIncompleteCompletions?: number;
}

export interface VscodeSuggestCompletionContext {
  triggerKind: number;
  triggerCharacter?: string;
}

export function normalizeVscodeSuggestCompletionContext(
  context: VscodeSuggestBridgeContext | null | undefined,
  monacoTriggerKinds: VscodeSuggestBridgeTriggerKinds | null | undefined,
): VscodeSuggestCompletionContext {
  if (context && monacoTriggerKinds && context.triggerKind === monacoTriggerKinds.TriggerCharacter) {
    return {
      triggerKind: VscodeCompletionTriggerKind.TriggerCharacter,
      triggerCharacter: context.triggerCharacter || undefined,
    };
  }
  if (context && monacoTriggerKinds && context.triggerKind === monacoTriggerKinds.TriggerForIncompleteCompletions) {
    return { triggerKind: VscodeCompletionTriggerKind.TriggerForIncompleteCompletions };
  }
  return { triggerKind: VscodeCompletionTriggerKind.Invoke };
}
