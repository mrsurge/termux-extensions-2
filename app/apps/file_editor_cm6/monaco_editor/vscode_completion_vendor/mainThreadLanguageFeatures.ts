/*
 * TE2 completion vendor shim. The full upstream VS Code file is preserved below
 * as comments so the app bundle only ships the completion adapter logic we use.
 * Source: worktrees/vscode-te2-diff/src/vs/workbench/api/browser/mainThreadLanguageFeatures.ts
 */
...
function labelInsertText(label: unknown): string {
  if (typeof label === 'string') return label;
  if (isRecord(label) && typeof label.label === 'string') return label.label;
  return '';
}

export interface InflateSuggestDtoDeps {
  propertyKind: number;
}

export function inflateSuggestDtoFromMainThreadLanguageFeatures(
  defaultRange: unknown,
  data: unknown,
  deps: InflateSuggestDtoDeps,
  ...
  return {
    label,
    kind: data[ISuggestDataDtoField.kind] ?? deps.propertyKind,
    ...
    insertText: data[ISuggestDataDtoField.insertText] ?? labelInsertText(label),
    range: data[ISuggestDataDtoField.range] ?? defaultRange,
    insertTextRules: data[ISuggestDataDtoField.insertTextRules],
    commitCharacters: typeof commitChars === 'string' ? Array.from(commitChars) : undefined,
...
export function inflateSuggestResultDtoFromMainThreadLanguageFeatures(
  dto: unknown,
  deps: InflateSuggestDtoDeps,
): { suggestions: Array<Record<string, unknown>>; incomplete: boolean; duration?: number } | null {
  if (!isRecord(dto)) return null;
  const completions = dto[ISuggestResultDtoField.completions];
  if (!Array.isArray(completions)) return null;

  const defaultRange = dto[ISuggestResultDtoField.defaultRanges];
  const suggestions: Array<Record<string, unknown>> = [];
  for (const rawItem of completions) {
    const item = inflateSuggestDtoFromMainThreadLanguageFeatures(defaultRange, rawItem, deps);
    if (item) suggestions.push(item);
  }

  const durationValue = dto[ISuggestResultDtoField.duration];
  return {
    suggestions,
    incomplete: !!dto[ISuggestResultDtoField.isIncomplete],
    duration: typeof durationValue === 'number' ? durationValue : undefined,
  };
}

/*
 * Upstream VS Code source, commented out intentionally.
 * Minification excludes this block from the served frontend bundle.
 */
