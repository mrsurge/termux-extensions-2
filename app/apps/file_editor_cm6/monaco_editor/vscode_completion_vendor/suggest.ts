/*
 * TE2 completion vendor shim. Full upstream VS Code suggest.ts is preserved
 * below as comments; the live exports here own the provider-side completion
 * path that TE2 uses from that file family.
...
import {
  ...
  type VscodeSuggestBridgeTriggerKinds,
} from './suggestModel.js';

interface VscodeSuggestBridgeModel {
  uri?: { toString(): string };
...
interface VscodeSuggestBridgePosition {
  lineNumber?: number;
  column?: number;
}

export interface VscodeWorkbenchCompletionRequest {
  uri: string;
  path: string;
  languageId: string;
...
function modelText(model: VscodeSuggestBridgeModel | null | undefined): string | undefined {
  try {
...
function modelVersion(model: VscodeSuggestBridgeModel | null | undefined): number | undefined {
  try {
...
export async function provideWorkbenchCompletionItemsFromVscodeSuggest(
  deps: VscodeSuggestWorkbenchProviderDeps,
