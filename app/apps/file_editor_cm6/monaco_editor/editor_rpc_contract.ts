...
export type EditorRpcMethodName =
  (typeof EDITOR_RPC_METHODS)[keyof typeof EDITOR_RPC_METHODS];
...
export const EDITOR_RPC_NOTIFICATIONS = {
  ...
  searchHighlight: "editor.search.highlight",
...
export type EditorRpcNotificationName =
  (typeof EDITOR_RPC_NOTIFICATIONS)[keyof typeof EDITOR_RPC_NOTIFICATIONS];
...
export interface EditorRpcFileOpenedNotificationParams extends Record<
  string,
  unknown
> {
  path?: string;
...
export interface EditorRpcGitBaselinesNotificationParams extends Record<
  string,
  unknown
> {
  path?: string;
...
export interface EditorRpcDraftDiffNotificationParams extends Record<
  string,
  unknown
> {
  path?: string;
...
export interface EditorRpcPrefsChangedNotificationParams extends Record<
  string,
  unknown
> {
...
export function isJsonRpcSuccessEnvelope(
  ...
  return (
    record.jsonrpc === JSONRPC_VERSION && "id" in record && "result" in record
  );
}

export function isJsonRpcErrorEnvelope(
  value: unknown,
): value is JsonRpcErrorEnvelope {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const record = value as Record<string, unknown>;
