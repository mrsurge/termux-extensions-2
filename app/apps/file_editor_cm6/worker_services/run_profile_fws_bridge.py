...
RUN_PROFILE_LABEL_PREFIXES = (
    "runner-profile:file_editor_cm6:",
    "page-preview:file_editor_cm6:",
)
...
def start_run_profile_fws_bridge() -> None:
    ...
    _connect_task = asyncio.create_task(
        _connect(client),
        name="file_editor_cm6_run_profile_fws_bridge",
    )
...
async def _on_connect() -> None:
    ...
    _snapshot_task = asyncio.create_task(
        _open_dashboard_snapshot(),
        name="file_editor_cm6_run_profile_fws_snapshot",
    )
...
async def _open_dashboard_snapshot() -> None:
    ...
    try:
        response = await client.call(
            ...
            {
                "jsonrpc": "2.0",
                "id": "file_editor_cm6_run_profile_snapshot",
...
async def _on_notification(payload: object) -> None:
    ...
    if not isinstance(method, str) or method not in FWS_LIFECYCLE_METHODS:
        return
...
def _replace_relevant_shell_ids(payload: object) -> None:
