...
function updateLspModalUI(state) {
  ...
  const pyrightMode = _getPyrightConfigMode(state);
  ...
  try {
    if (lspModal.pyrightModeRoot) lspModal.pyrightModeRoot.checked = pyrightMode === 'root';
    if (lspModal.pyrightModeWorkers) lspModal.pyrightModeWorkers.checked = pyrightMode === 'workers';
    if (lspModal.pyrightModeRoot) lspModal.pyrightModeRoot.disabled = !isEnabled;
    if (lspModal.pyrightModeWorkers) lspModal.pyrightModeWorkers.disabled = !isEnabled;
    if (lspModal.pyrightWorkersBtn) {
      lspModal.pyrightWorkersBtn.disabled = !isEnabled || pyrightMode !== 'workers';
    }
  } catch (err) {
...
window.__cm6HandleLspStatus = (payload) => {
  try {
    ...
    // Best-effort: keep modal updated even if it's open already.
    if (typeof editorViewState === 'object' && editorViewState) {
...
async function _startStopLspServer(serverId) {
  ...
  // Best-effort immediate refresh (websocket is the source of truth).
  try {
    const cache = await apiGet('api/lsp/debug/cache');
    _setLastLspStatus(_deriveLspStatusFromCache(cache));
  } catch { }
...
function _addPyrightWorkerRow(worker = {}) {
  ...
  pickConfigBtn.addEventListener('click', async () => {
    const projectRootAbs = pyrightWorkersState.projectRootAbs || (cachedProjectRoot ? toAbsolute(cachedProjectRoot, null, HOME_DIR) : '');
...
async function _savePyrightWorkers() {
  ...
  try {
    const workers = _collectPyrightWorkersFromUI();
    ...
    if (errors && errors.length) {
      host.toast(`Saved with ${errors.length} issue(s)`);
    } else {
      host.toast('Pyright workers saved');
    }
  } catch (e) {
...
function _joinLines(items) {
  ...
  return items.map(v => String(v)).filter(Boolean).join('\n');
}
...
function _setPyrightTypeMode(value) {
  ...
  _buildPyrightTypeDropdown(mode);
}

function _buildPyrightTypeDropdown(current) {
...
function _resolveWorkerRootAbs(rootRel) {
  const base = pyrightConfigState.projectRootAbs || pyrightWorkersState.projectRootAbs || cachedProjectRoot || HOME_DIR;
...
async function _savePyrightConfig() {
  ...
  // If worker row doesn't have a project config set, set it to this file.
...
// Replay any prompt queued before main.js installed the handler.
try {
...
function closeAllMenus() {
  ...
  try {
    ...
    if (pyrightConfigModal && pyrightConfigModal.typeDD) {
      pyrightConfigModal.typeDD.classList.remove('show');
    }
  } catch { }
