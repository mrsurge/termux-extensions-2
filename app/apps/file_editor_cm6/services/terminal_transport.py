...
async def _proxy_terminal_socketio_websocket(websocket: WebSocket, rest: str = "") -> None:
    ...
    incoming_path = websocket.scope.get("path") or "/terminal_ws/socket.io"
    if incoming_path == "/terminal_ws/socket.io":
        incoming_path = "/terminal_ws/socket.io/"
    ...
    try:
        ...
        print(f"[Terminal WS] Failed to proxy to worker_url={worker_url}: {exc}", file=sys.stderr, flush=True)
    finally:
...
def register(app) -> None:
    app.add_api_websocket_route(
        ...
        name="terminal_ws_proxy_root",
    )
    app.add_api_websocket_route(
        "/terminal_ws/socket.io/{rest:path}",
        _proxy_terminal_socketio_websocket,
        name="terminal_ws_proxy",
    )
