...
async def _find_vscode_api_shell(shell_id: str | None) -> tuple[str, int] | None:
    ...
    if not record:
        # Best-effort fallback: find any running vscode_api shell for file_editor_cm6.
        try:
        ...
        for s in shells:
            ...
            if not s.label:
                continue
            if s.label.startswith("vscode_api:file_editor_cm6"):
                record = s
                break
    ...
    port_s = env.get("TE_VSCODE_API_PORT") or env.get("PORT") or ""
    try:
        port = int(str(port_s))
    except Exception:
        return None
...
async def _proxy_vscode_api_websocket(websocket: WebSocket) -> None:
    """Main-process shim: proxy browser WS to the vscode_api framework shell WS.
    ...
    found = await _find_vscode_api_shell(requested_shell_id)
    if not found:
    ...
    upstream_url = f"ws://127.0.0.1:{port}/vscode_api"
    ...
    try:
        ...
        print(f"[vscode_api WS] Failed proxy upstream_url={upstream_url}: {exc}", file=sys.stderr, flush=True)
    finally:
...
def register(app) -> None:
    """Register vscode_api websocket proxy in the main process."""

    app.add_api_websocket_route(
        "/vscode_api_ws",
        _proxy_vscode_api_websocket,
        name="vscode_api_ws_proxy",
    )
