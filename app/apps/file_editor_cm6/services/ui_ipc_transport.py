...
async def _proxy_ui_ipc_socketio_websocket(websocket: WebSocket, rest: str = "") -> None:
    """Main-process shim: proxy /ui_ipc_ws Socket.IO (websocket transport) to the worker.
    ...
    incoming_path = websocket.scope.get("path") or "/ui_ipc_ws/socket.io"
    if incoming_path == "/ui_ipc_ws/socket.io":
        incoming_path = "/ui_ipc_ws/socket.io/"
    ...
    try:
        ...
        print(f"[UI IPC WS] Failed to proxy to worker_url={worker_url}: {exc}", file=sys.stderr, flush=True)
    finally:
...
def register(app) -> None:
    """Register the UI IPC Socket.IO websocket proxy in the main process."""

    app.add_api_websocket_route(
        "/ui_ipc_ws/socket.io",
        _proxy_ui_ipc_socketio_websocket,
        name="ui_ipc_ws_proxy_root",
    )
    app.add_api_websocket_route(
        "/ui_ipc_ws/socket.io/{rest:path}",
        _proxy_ui_ipc_socketio_websocket,
        name="ui_ipc_ws_proxy",
    )
