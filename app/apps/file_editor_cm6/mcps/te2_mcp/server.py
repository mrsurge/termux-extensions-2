...
def build_server() -> FastMCP:
    ...
    async def te2_fws_running(include_stats: bool = True) -> dict:
        return await framework_shells.get_running(include_stats=include_stats)

    @server.tool(description="Return framework-shell detail directly from framework_shells manager.")
    async def te2_fws_shell_get(shell_id: str, include_stats: bool = True) -> dict:
        return await framework_shells.get_shell(shell_id=shell_id, include_stats=include_stats)
