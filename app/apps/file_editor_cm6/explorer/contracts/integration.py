...
def parse_cm6_mirror_params(payload: object) -> JsonObject:
    envelope = _as_object(payload)
    path = envelope.get("path")
    content = envelope.get("content")
    if not isinstance(path, str) or not path:
        raise ExplorerIntegrationContractError("Missing path")
    if not isinstance(content, str):
        raise ExplorerIntegrationContractError("Missing content")
    return envelope
