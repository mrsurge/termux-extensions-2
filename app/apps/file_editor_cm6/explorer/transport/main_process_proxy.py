"""Main-process proxy for the worker-owned Explorer Socket.IO endpoint."""
...
async def _proxy_explorer_socketio_websocket(websocket: WebSocket, rest: str = "") -> None:
    """Main-process shim: proxy /explorer_ws Socket.IO (websocket transport) to the worker.
    ...
    incoming_path = websocket.scope.get("path") or "/explorer_ws/socket.io"
    if incoming_path == "/explorer_ws/socket.io":
        incoming_path = "/explorer_ws/socket.io/"
    ...
    try:
        ...
        print(f"[Explorer WS] Failed to proxy to worker_url={worker_url}: {exc}", file=sys.stderr, flush=True)
    finally:
...
def register(app) -> None:
    """Register the explorer Socket.IO websocket proxy in the main process."""

    app.add_api_websocket_route(
        "/explorer_ws/socket.io",
        _proxy_explorer_socketio_websocket,
        name="explorer_ws_proxy_root",
    )
    app.add_api_websocket_route(
        "/explorer_ws/socket.io/{rest:path}",
        _proxy_explorer_socketio_websocket,
        name="explorer_ws_proxy",
    )
