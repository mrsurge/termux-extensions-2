 ...
 * }} deps
 */
export function createMenuCoreController(deps) {
  ...
  function installPrimaryMenuButtons() {
    ...
    deps.menuFileBtn.addEventListener('click', (e) => { e.stopPropagation(); const open = deps.menuFileDD.classList.toggle('show'); if (open) closeOthers(deps.menuFileDD); });
    deps.menuEditBtn.addEventListener('click', (e) => { e.stopPropagation(); const open = deps.menuEditDD.classList.toggle('show'); if (open) closeOthers(deps.menuEditDD); });
    deps.menuEditorBtn.addEventListener('click', (e) => { e.stopPropagation(); const open = deps.menuEditorDD.classList.toggle('show'); if (open) closeOthers(deps.menuEditorDD); });
    deps.menuViewBtn.addEventListener('click', (e) => { e.stopPropagation(); const open = deps.menuViewDD.classList.toggle('show'); if (open) closeOthers(deps.menuViewDD); });
    deps.recentFilesBtn.addEventListener('click', (e) => { e.stopPropagation(); const open = deps.recentFilesDD.classList.toggle('show'); if (open) closeOthers(deps.recentFilesDD); });
    deps.runActiveBtn.addEventListener('click', (e) => { e.stopPropagation(); deps.runCurrentFile(); });
