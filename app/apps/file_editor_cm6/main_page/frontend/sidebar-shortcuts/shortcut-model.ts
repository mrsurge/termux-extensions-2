...
export function collectShortcuts(uiPrefs: UnknownRecord): SidebarShortcut[] {
  ...
  raw.forEach((entry) => {
    ...
    const url = normStr(sc.url) || (kind === SHORTCUT_KIND_FRAMEWORK_APP ? buildFrameworkAppUrl(appId) : '');
    if (!url) return;
...
export function resolveActive(
  ...
  const activeId = normStr(clientActiveShortcutId) || normStr(uiPrefs?.[UI_PREF_KEY_ACTIVE]);
...
export function pickMruShortcut(shortcuts: SidebarShortcut[]): SidebarShortcut | null {
