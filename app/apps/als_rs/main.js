...
function _resolveAppIdFromPath() {
  ...
  throw new Error(`Unable to resolve app id from path '${window.location.pathname}'`);
}
...
export default async function initProxyShellApp(rootEl, _api, host) {
  ...
  const metaResponse = await fetch(metaUrl, { method: 'GET', cache: 'no-store' });
  if (!metaResponse.ok) {
    throw new Error(`Failed to load proxy shell config for ${appId} (${metaResponse.status})`);
  }
  ...
  while (Date.now() - startedAt < MAX_WAIT_MS) {
    try {
      const healthResponse = await fetch(data.health_url, { method: 'GET', cache: 'no-store' });
