...
interface ExplorerRpcRuntimeGlobals {
  ...
  __codeTe2HandleUiPrefs?: (payload: JsonObject) => void;
  __codeTe2HandleWatcherConfig?: (payload: JsonObject) => void;
  __codeTe2HandleWatcherError?: (payload: JsonObject) => void;
  __codeTe2HandleWatcherModeStatus?: (payload: JsonObject) => void;
  __codeTe2HandleWatcherRaiseResult?: (payload: JsonObject) => void;
  __codeTe2PendingUiPrefs?: JsonObject | null;
}
...
export function createExplorerRpcRuntime(deps: ExplorerRpcRuntimeDeps): ExplorerRpcRuntime {
  ...
  function handleExplorerRpcNotification(
    ...
    if (
      ...
      && typeof win.__codeTe2HandleWatcherConfig === 'function'
    ) {
      win.__codeTe2HandleWatcherConfig(payload);
    }
    if (
      method === EXPLORER_RPC_NOTIFICATIONS.watcherModeStatus
      && typeof win.__codeTe2HandleWatcherModeStatus === 'function'
    ) {
      win.__codeTe2HandleWatcherModeStatus(payload);
    }
    if (
      method === EXPLORER_RPC_NOTIFICATIONS.watcherError
      && typeof win.__codeTe2HandleWatcherError === 'function'
    ) {
      win.__codeTe2HandleWatcherError(payload);
    }
    if (
      method === EXPLORER_RPC_NOTIFICATIONS.watcherLimitRaiseResult
      && typeof win.__codeTe2HandleWatcherRaiseResult === 'function'
    ) {
      win.__codeTe2HandleWatcherRaiseResult(payload);
    }
    if (method === EXPLORER_RPC_NOTIFICATIONS.prefsUiUpdated) {
      if (typeof win.__codeTe2HandleUiPrefs === 'function') {
        win.__codeTe2HandleUiPrefs(payload);
      } else {
        try { win.__codeTe2PendingUiPrefs = payload; } catch {}
      }
    }
