...
_metrics_enabled = _env_flag("CODE_TE2_EVENT_METRICS")
_metrics_interval_s = _env_float("CODE_TE2_EVENT_METRICS_INTERVAL_S", 30.0)
_metrics_slow_handler_ms = _env_float(
    "CODE_TE2_EVENT_METRICS_SLOW_HANDLER_MS",
    25.0,
)
_metrics_slow_queue_ms = _env_float("CODE_TE2_EVENT_METRICS_SLOW_QUEUE_MS", 50.0)
...
def _ensure_metrics_summary_task(loop: asyncio.AbstractEventLoop) -> None:
    ...
    if _metrics_summary_task is None or _metrics_summary_task.done():
        _metrics_summary_task = loop.create_task(
            ...
            name="code_te2_event_bus_metrics",
        )
