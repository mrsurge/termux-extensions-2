...
import { requestExplorerRpc } from '../../../src/explorer/rpc/client.ts';
...
interface LspStatusArgs {
  show?: boolean;
  languageId?: string;
  state?: string;
  payload?: LspStatusPayload;
}

export function createAdapterUiController(deps: any) {
  ...
  function setSpinnerClass(el: HTMLElement, cls: string) {
  ...
  function handleLspStatusUpdate(args: LspStatusArgs = {}) {
    const { show, languageId, state, payload } = args;
    try {
      if (!window.__feLspSpinnerUi) {
        window.__feLspSpinnerUi = {
          lspShow: false, lspTitle: '', busyShow: false, busyTitle: '', busyLanguageId: '', busyActivity: '',
  ...
  function installLspStatusHandler() {
    window.__codeTe2HandleLspStatusUpdate = handleLspStatusUpdate;
  }
