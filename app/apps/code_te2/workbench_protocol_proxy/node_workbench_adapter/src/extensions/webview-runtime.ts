...
function applyServerEvent(event,fromResume=false){if(disposed||!event||event.surfaceId!==surfaceId)return;if(reconciling&&!fromResume){bufferServerEvent(event);return;}const epoch=String(event.serverEpoch||'');const generation=String(event.surfaceGeneration||'');const revision=Number(event.htmlRevision);const sequence=Number(event.sequence);if(!Number.isSafeInteger(sequence))return;if(transport.loaded&&epoch===transport.serverEpoch&&generation===transport.surfaceGeneration&&sequence<=transport.lastServerSequence)return;if(!transport.loaded||epoch!==transport.serverEpoch||generation!==transport.surfaceGeneration||revision!==transport.htmlRevision){bufferServerEvent(event);void reconcileTransport();return;}if(sequence!==transport.lastServerSequence+1){bufferServerEvent(event);void reconcileTransport();return;}transport.lastServerSequence=sequence;if(event.event==='message')deliver('message',deserialize(event.jsonMessage,event.buffers));else if(event.event==='reload')void reconcileTransport();else if(event.event==='dispose'){disposed=true;status.hidden=false;status.textContent='Extension view closed';frame.remove();rejectPending(new Error('Extension view closed'));}}
