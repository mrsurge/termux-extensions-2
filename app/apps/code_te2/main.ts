// /data/data/com.termux/files/home/mrselect/app/apps/code_te2/main.ts
...
export default async function initFileEditor(rootEl: HTMLElement, api: HostApi, host: HostBridge) {
  ...
  createSettingsBootstrap({
    els: {
      ...
      settingsCursorStyle: hostElements.editorSettingsCursorStyle,
      extConfigModal,
      extConfigTitle,
      extConfigForm,
      extConfigClose,
  ...
  window.__codeTe2HandleLspStatusUpdate = adapterUi.handleLspStatusUpdate;
  ...
  window.addEventListener('code-te2:sidebar-project-opened', (event) => {
  ...
  const cacheIndicatorController = createCacheIndicatorController({
    ...
    confirmDiscard: (path: string) => window.teUI.dialog.confirm(
      `Discard the unsaved draft for “${basename(path)}”? The editor will restore the last saved file contents.`,
      {
        title: 'Discard Draft?',
        severity: 'danger',
        confirmLabel: 'Discard Draft',
        cancelLabel: 'Keep Draft',
        surface: { id: 'code-te2.draft-discard' },
      },
    ),
  ...
  const openFlowController = createOpenFlowController({
    ...
    getRestoredSessionActive: () => !!restoredSessionActive,
    getCurrentPath: () => currentPath,
    setRestoredSessionActive: (flag: boolean) => {
      restoredSessionActive = flag;
    },
    ...
    setCurrentPathExists: (exists: boolean) => {
      currentPathExists = exists;
    },
    setLastPickerPath: (path: string) => {
      lastPickerPath = path;
    },
    parentDir,
  ...
  const saveFlowController = createSaveFlowController({
    ...
    setEditorState: (state: unknown) => {
      editorState = state && typeof state === 'object' ? (state as EditorState) : null;
    },
    setCachedProjectRoot: (path: string | null) => {
      cachedProjectRoot = path;
      void sidebarShortcuts?.setProjectRoot?.(path || '');
  ...
  renderRunProfileState();
  window.addEventListener('code-te2:run-profile-state-changed', (event) => {
  ...
  async function quitThisFrameworkApp(): Promise<void> {
    try {
      const resp = await fetch('/api/apps/code_te2/quit', {
        method: 'POST',
