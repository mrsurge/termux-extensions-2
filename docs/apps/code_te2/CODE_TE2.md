...
### Pinned WBA protocol map

WBA imports `src/protocol/pinned-rpc-ids.ts` through `rpc-ids.ts`. The map is
explicitly shipped by `build.mjs` as `dist/protocol/pinned-rpc-ids.mjs` beside the
adapter modules. It is frozen, and is the sole authority, not a fallback.
It covers the 30 currently consumed actors for code-server 4.130.0, Code commit
`197ef3e8da8ee99ed6ca8f1a630157527e6d448f`.

Adding an actor requires adding its exact NID to this source map. Upgrading the
managed Code build requires explicitly reviewing/updating the map, its pin/commit,
and protocol tests together. NIDs follow the exact pinned Code protocol declaration
order; they are not assigned independently by extension or interface name.

There is no runtime NID parsing, `code-server --version` probe, generated JSON
config, environment override, or stale/default-map fallback. Legacy
`te2_rpc_config.json` files left on disk are ignored. Named runtime-adapter fields
continue to supply the actor IDs to intelligence, commands, storage and webviews.
Connect logging identifies the pinned map/version/commit. Tests in
`tests/extension_storage_runtime.test.mjs` verify stale config cannot override it.
...
The private Code Server installer is pinned to Code Server 4.130.0 and serialized by a cache-scoped filesystem lock. Startup never downloads or probes the executable; installation only runs after user confirmation through the existing UI IPC path. Successful launch can persist a pre-ledger installation migration.
...
- Explicit installation validates the exact private `lib/code-server-4.130.0/lib/vscode` tree. It does not walk launcher parents to `/lib`, where a system Code Server installation could otherwise be mistaken for the private runtime's bundled workbench.
...
- The persisted Termux layout identifies that launcher's adjacent `lib/code-server/lib/vscode` tree directly rather than evaluating its shell-local `$ROOT`; existing managed prefixes do not require reinstallation.
...
- WBA receives the exact extension manifest and User settings paths through its
  Framework-Shell environment; protocol IDs ship in its imported map. It never probes
...
- Framework-Shells 0.0.64 release wheels are native: ordinary CPython uses
  `cp39-abi3`, free-threaded CPython uses its exact `cp314-cp314t` tag, and
  both carry the PyO3 pump plus Rust terminal broker. Agent Log Server 0.2.131
...
failure does not. Switching to web workers also clears it but preserves the package
on disk. Switching back uses the consented installer path to validate/adopt that
preserved package rather than forcing a download.

An absent record denotes migration: with Code Server mode enabled, try the known
platform-specific managed launcher once. Success records the pin/layout; launch
