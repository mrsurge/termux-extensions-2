...
| `app/apps/code_te2/template.html` | Added Socket.IO CDN; restructured git footer (two rows) |
| `app/apps/code_te2/main.js` | WebSocket connection setup; removed redundant refresh calls |
| `app/apps/code_te2/explorer_ws.py` | **New** — WS dispatcher, connection manager, message handlers, broadcasts |
| `app/apps/code_te2/explorer_helper.py` | Draft cache (5s TTL), git flags, performance instrumentation |
| `app/apps/code_te2/static/js/explorer.js` | WS-driven rendering, status propagation, hunk formatting |
| `app/apps/code_te2/static/js/explorer.css` | Git status styling, draft indicators, `fe-dir-has-*` classes |
| `app/apps/code_te2/explorer/search.py` | **New** — Name/content/changes search implementations |
| `app/apps/code_te2/explorer/review.py` | **New** — Draft management, bulk save/discard |
| `app/apps/code_te2/core_read.py` | Explorer notification hooks in file watcher |
| `app/apps/code_te2/main.py` | WebSocket route integration, draft state notifications |
| `app/apps/code_te2/nicegui_editor/editor_app.py` | Draft state notifications on save/discard |
