...
| Implement `/app/{app_id}` shell rendering | Partial | Rust spike now serves the framework `app_shell.html` wrapper for standard apps, rewrites the minimal template placeholders, supports `/api/state`, and the shell consumes `/ws/apps` readiness/running updates instead of repeating readiness polling. Settings endpoints are intentionally out of this MVP slice. |
