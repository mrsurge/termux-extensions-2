...
## Remote Model Forms
- `template.html` now includes shared **Display Name** support plus remote-only inputs for **Model Identifier** (`remote_model`) and **Reasoning Effort** (stored as trimmed strings such as “low”, “medium”, “high”).
...
## Backend Additions
- Manifest writer persists `remote_model` and `reasoning_effort` (always as strings) for API-backed models.
