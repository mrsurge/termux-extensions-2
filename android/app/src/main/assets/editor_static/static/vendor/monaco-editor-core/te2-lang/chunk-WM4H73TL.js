import {
  createWebWorker
} from "./chunk-JEXTF4RQ.js";
import {
  typescriptDefaults
} from "./chunk-JUXMGXZO.js";
...
var LibFiles = class {
  ...
  getOrCreateModel(fileName) {
    ...
    const model = editor2.getModel(uri);
    if (model) {
      return model;
    }
    if (this.isLibFile(uri) && this._hasFetchedLibFiles) {
      return editor2.createModel(this._libFiles[uri.path.slice(1)], "typescript", uri);
    }
    const matchedLibFile = typescriptDefaults.getExtraLibs()[fileName];
    if (matchedLibFile) {
      return editor2.createModel(matchedLibFile.content, "typescript", uri);
    }
    return null;
  }
...
var DiagnosticsAdapter = class extends Adapter {
  constructor(_libFiles, _defaults, _selector, worker) {
      ...
      const visibleSubscription = model.onDidChangeAttached(() => {
        ...
        if (onlyVisible) {
          if (model.isAttachedToEditor()) {
            ...
            editor2.setModelMarkers(model, this._selector, []);
          }
        }
      });
    ...
    this._disposables.push(
      editor2.onDidCreateModel((model) => onModelAdd(model))
    );
    this._disposables.push(editor2.onWillDisposeModel(onModelRemoved));
    this._disposables.push(
      editor2.onDidChangeModelLanguage((event) => {
    ...
    this._disposables.push({
      dispose() {
        for (const model of editor2.getModels()) {
    ...
    editor2.getModels().forEach((model) => onModelAdd(model));
  }
  dispose() {
...
//# sourceMappingURL=chunk-WM4H73TL.js.map
